"""
    This function is used to construct an intelligent response to a given message. It uses the find_max_sentiment and key_phrase_finder functions to firstly, 
    find the overwhelming sentiment of a message and secondly, see if the key phrases determined by AWS Comprehend matches with a supplied list of phrases. 
    Given the sentiment and the boolean output of the key_phrase_finder function, we can then use conditional logic to craft an intelligent response.
